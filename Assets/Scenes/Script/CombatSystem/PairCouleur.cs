...
public class PairCouleur : MonoBehaviour
{
    ...
    public bool CouleurComplementaire(Color color1, Color color2)
    {

        if (!_couleur.TryGetValue(color1, out string nomCouleur1) || !_couleur.TryGetValue(color2, out string nomCouleur2))
        {           
            return false;
        }

        if (_pairs.TryGetValue(nomCouleur1, out string nomComplementaire2))
        {
        ...
        {
            if (nomComplementaire1 == nomCouleur1)
            {
                return true;
            }
        }
